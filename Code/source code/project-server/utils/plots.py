...
def cv2ImgAddText(img, text, left, top, textColor=(0, 255, 0), textSize=20):
    ...
    fontText = ImageFont.truetype(
        r"/usr/share/fonts/wqy-microhei/wqy-microhei.ttc", textSize, encoding="utf-8")
...
def plot_one_box(x, im, color=(128, 128, 128), label=None, line_thickness=3):
    ...
    if label:
        ...
        c2 = c1[0] + 240, c1[1] - t_size[1] - 3
        cv2.rectangle(im, c1, c2, color, -1, cv2.LINE_AA)  # filled
        # change
        im = Image.fromarray(cv2.cvtColor(im, cv2.COLOR_BGR2RGB))
        draw = ImageDraw.Draw(im)
        fontText = ImageFont.truetype(
            r"/usr/share/fonts/wqy-microhei/wqy-microhei.ttc", 35, encoding="utf-8")
        draw.text((c1[0], c1[1] - 35), label, (225, 255, 255), font=fontText)
